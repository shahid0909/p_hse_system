...
class ChemicalRegisterController extends Controller
{
    ...
    public function index(){
        ...
        return view('dashboards.admins.chemicalRegister.index', compact('user', 'chemicals','ppe'));
    }
    ...
    public function chemicalData($id){
        $chemicalData = DB::table('chemical as c')
            ...
            ->select('C.chemical_Name AS chemical_name','ph.physicalform',
                ...
                's.SupplierName','s.contactNo','s.Email','s.address', 'm.name AS manufacture', 'cs.caseName','cs.ingredient', 'h.image AS hazard_image')
            ->where('c.id', $id)
            ->get();
